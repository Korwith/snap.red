...
class Dropdown {
    ...
    setPrimaryOption(option, skip_select) {
    ...
    toggle(force) {
        if (force !== false && this.element.classList.contains('empty'))
            return;
    ...
    updateHeight() {
        const visible_count = this.options.filter(o => !o.element.classList.contains('hidden')).length;
        this.element.classList.toggle('empty', visible_count <= 1);
        const calculated_expanded = this.height * visible_count;
